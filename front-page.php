...
<main class="main main-index">
	...
	<section class="main-screen">
		<div class="main-screen__main">
			...
			<?php if ( have_rows( 'ms_row' ) ) : ?>
				...
				<?php
				$cr_item = 1;
				while ( have_rows( 'ms_row' ) ) :
					the_row();

					if ( 2 === $cr_item ) :
						$marquee_class = 'marquee-item-rev';
					else :
						$marquee_class = 'marquee-item';
					endif;
					...
					<div
						class="main-screen__credits-row <?php echo esc_attr( $marquee_class ); ?>"
						data-marque-speed="<?php echo esc_attr( $marque_speed ); ?>"
					>
						<h2 class="main-screen__credits-item marquee-itemchild">
						<?php
						...
						if ( $ms_text ) :
							...
							foreach ( (array) $ms_array as $ms_item ) :
								if ( strpos( $ms_item, 'b:' ) === 0 ) :
									echo '<span class="black">' . esc_html( str_replace( 'b:', '', $ms_item ) ) . '</span>';
								else :
									echo '<span>' . esc_html( $ms_item ) . '</span>';
								endif;
							endforeach;
			...
			<div class="container main-screen__image-wrapper" id="scene">
				<div class="main-screen__image" data-depth="0.6">
				<?php
				$ms_image_1 = get_field( 'ms_image_1' );
				if ( $ms_image_1 ) :
					echo '<img src="' . esc_url( kama_thumb_src( 'src=' . $ms_image_1 . '&w=249&h=417' ) ) . '" width="249" height="417" srcset="' . esc_url( kama_thumb_src( 'src=' . $ms_image_1 . '&w=498&h=834' ) ) . ' 2x" alt="Жест \'Коза\'"/>';
				endif;

				$ms_image_2 = get_field( 'ms_image_2' );
				if ( $ms_image_2 ) :
					echo '<img src="' . esc_url( kama_thumb_src( 'src=' . $ms_image_2 . '&w=249&h=417' ) ) . '" width="249" height="417" srcset="' . esc_url( kama_thumb_src( 'src=' . $ms_image_2 . '&w=498&h=834' ) ) . ' 2x" alt="Жест \'Мир\'"/>';
				endif;
			...
			<div class="container main-screen__image-text">
				<?php the_field( 'ms_caption' ); ?>
			</div>
		</div>

		<?php if ( is_front_page() || is_page_template( 'page-foreign.php' ) ) : ?>
		<div class="consulting anim-item anim-item--no-hide">
		...
		<div class="main-screen__nav">
			...
			if ( $phone ) :
				echo '<a class="main-screen__tel" href="tel:' . esc_url( preg_replace( '/[^0-9\+]/', '', $phone ) ) . '">' . esc_html( $phone ) . '</a>';
			endif;
			unset( $phone );
			?>
		...
		<div class="about-teaser">
			...
			<div class="about-teaser__bottom">
				<div class="about-teaser__images-wrapper">
					<div class="about-teaser__images" id="scene2">
					<?php
					$about_image_1 = get_field( 'about_image_1' );
					if ( $about_image_1 ) :
						echo '<img class="about-teaser__image-bust" src="' . esc_url( kama_thumb_src( 'src=' . $about_image_1 . '&w=268&h=522' ) ) . '" data-depth="0.6" width="268" height="522" srcset="' . esc_url( kama_thumb_src( 'src=' . $about_image_1 . '&w=536&h=1044' ) ) . ' 2x" alt="Бюст"/>';
					endif;

					$about_image_2 = get_field( 'about_image_2' );
					if ( $about_image_2 ) :
						echo '<img class="about-teaser__image-shape" src="' . esc_url( $about_image_2 ) . '" data-depth="-0.6" width="154" height="154" alt="Фигура"/>';
					endif;
		...
		<div class="history-scroll">
			<section class="history-events history-scroll__block">
				...
				<div class="history-events__title">
					...
					if ( $prehistory_title ) :
						$i                    = 1;
						$prehistory_title_arr = preg_split( '/<br[^>]*>/i', $prehistory_title );
						foreach ( $prehistory_title_arr as $prehistory_title_item ) :
							?>
							<div class="history-events__title-row anim-item anim-item--no-hide anim-left">
								<div
									class="history-events__title-item
									<?php
									if ( 3 === $i ) :
										echo ' history-events__title-item--flex';
									endif;
									?>
									"
								>
							...
							$i++;
						endforeach;
						unset( $prehistory_title, $i, $prehistory_title_arr, $prehistory_title_item );
					endif;
					?>
				</div>
				<div class="history-events__info">
					<?php
					$prehistory_text = get_field( 'prehistory_text' );
					if ( $prehistory_text ) :
						$prehistory_text_arr = preg_split( '/<br[^>]*>/i', $prehistory_text );
						// @codingStandardsIgnoreStart
						echo '<div class="history-teaser__info-block">' . join( '</div><div class="history-teaser__info-block">', $prehistory_text_arr ) . '</div>';
						// @codingStandardsIgnoreEnd
						unset( $prehistory_text, $prehistory_text_arr );
					endif;
					?>
				</div>
			</section>

			<section class="history-award history-scroll__block">
				<h2 class="history-award__title anim-item anim-item--no-hide anim-bottom"><?php the_field( 'awards_title' ); ?></h2>
				<?php
				$awards = get_field( 'awards' );
				if ( $awards ) :
					?>
				<ul class="history-award__list">
					<?php
					foreach ( $awards as $award ) :
						echo '<li class="history-award__item anim-item anim-item--no-hide anim-left"><img src="' . esc_url( $award ) . '" alt=""/></li>';
					endforeach;
					?>
				</ul>
					<?php
			...
			<section class="history-final history-scroll__block" style="background-image: url( <?php the_field( 'awards_img_bg' ); ?> );">
				...
				<?php if ( get_field( 'awards_count' ) ) { ?>
					<div class="history-final__block anim-item anim-item--no-hide anim-scale-opacity">
						...
						<div class="history-final__caption"><?php the_field( 'awards_count_text' ); ?></div>
					</div>
				<?php } ?>
			</section>
		</div>
	...
	<section class="history-award history-award--mobile">
		<h2 class="history-award__title anim-item anim-item--no-hide anim-bottom"><?php the_field( 'awards_title' ); ?></h2>
		<?php
		$awards = get_field( 'awards' );
		if ( $awards ) :
	...
	<section class="clients">
		<div class="container clients__wrapper">
			...
			<h2 class="clients__title history-award__title anim-item anim-item--no-hide anim-bottom active"><?php the_field( 'clients_title' ); ?></h2>
			<?php
			...
			if ( $clients ) :
				...
				<ul class="clients__list">
					...
					foreach ( $clients as $client ) :
						?>
					<li class="clients__item clients__item-<?php echo esc_attr( $num_img_client ); ?> anim-item anim-item--no-hide anim-top">
						<?php
						foreach ( $client as $logo ) :
							echo '<img src="' . esc_url( $logo ) . '" width="200" height="100" alt="Логотип"/>';
						endforeach;
						?>
					</li>
						<?php
